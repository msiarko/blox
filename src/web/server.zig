...
pub const Request = struct {
    ...
    pub fn upgradeWebsocket(self: *@This()) !WebSocket {
        ...
        switch (upg) {
            .websocket => |key| {
                if (key) |k| {
                    return try self.http_req.respondWebSocket(.{ .key = k });
                }
            },
            else => {},
        }
    ...
    return struct {
        ...
        fn addRoute(self: *Self, method: std.http.Method, path: []const u8, handler: Handler) !void {
            ...
            if (!res.found_existing) {
                res.value_ptr.* = .{
                    .handlers = std.AutoHashMap(std.http.Method, Handler).init(self.allocator),
                };
            }
    ...
    return struct {
        ...
        pub fn listen(self: *Self, port: u16) !void {
            ...
            while (true) {
                ...
                const task = try self.ctx.io.concurrent(handleConnection, .{
                    self,
                    conn,
                });
        ...
        fn handleConnection(
            ...
            defer conn.socket.close(server.ctx.io);
        ...
        fn handleRequest(ctx: *Context(State), router: *const Router(State), req: Request) !void {
            ...
            if (router.routes.get(target)) |route_entry| {
                ...
                return req.http_req.respond("Not Found", .{ .status = .not_found });
            }
        }
    };
}
